...
/*   Updated: 2019/05/08 13:49:33 by sgalasso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "editor.h"

static void		export_vertex(int fd, t_pos min, t_env *env)
{
	t_vtx		*vtx;
	int			i;
	...
	while (vtx)
	{
		vtx->id = i;
		dprintf(fd, "v %d %d\n",
			(int)(vtx->pos.x - min.x), (int)(vtx->pos.y - min.y));
		vtx = vtx->next;
		i++;
	}
}

static void		export_sectors(int fd, t_env *env)
{
	t_sct		*sct;
	t_w_vtx		*w_vtx;
	int			i;
...
static void		export_objects(int fd, t_pos min, t_env *env)
{
	t_object	*obj;
	...
	while (obj)
	{
		if (obj->dd != DD_NTTY && obj->dd != DD_SPEC)
			...
			(int)(obj->pos.x - min.x), (int)(obj->pos.y - min.y),
